...
#python import_csv.py --settings=sgp.suablue.settings --clientes=Conv-CSV_IMPORTACAO_SGP_BLUENET.csv --planoadd=1 --vencimentoadd=1 --portador=1 --nas=1 --sync=1
    ...
    with open(args.clientes, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            if plano=='':
                plano ='defautl'
            try:
                plano_valor = int(str(row[19]).replace('R$', '').replace(',','.'))
            
            except:
                plano_valor=0
            try:
                plano_download= int(fnum(row[26]))
            except:
                plano_download=0
            try:
                plano_upload= int(fnum(row[27]))
            except:
                plano_upload=0
