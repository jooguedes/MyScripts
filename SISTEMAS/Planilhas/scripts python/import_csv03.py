...
#python import_csv.py --settings=sgp.netminasfibra.settings --clientes=Conv-Clientes_minas_info.csv --planoadd=1 --vencimentoadd=1 --portador=1 --nas=1 --sync=1
...
if args.clientes:
    ...
    with open(args.clientes, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            ri += 1
            #idcontrato = int(row[8]) 
            #idCliente= int(row[0])
            #'%s@import'%
            login= ustrl(row[21]) 
            if login =='':
                login=str(row[2])+'sem_login'
            nome = ustr(row[0])
            cpfcnpj = ustr(row[2])[:20]
            rgie = ustr(row[3])[:20]
            sexo = None
            ...
            logradouro = ustr(row[11])
            numero = None
            complemento = row[13]
            bairro = ''
            cep = ustr(row[15]).strip()[0:20]
            uf = ustr(row[17])
            cidade = ustr(row[16]).upper()[0:50]
            ...
            if plano=='':
                plano ='defautl'
            try:
                plano_valor = int(str(row[19]).strip())
            
            except:
                plano_valor=0
            try:
                plano_download=int(fnum(row[26]))*1024
            except:
                plano_download=0
            try:
                plano_upload= int(fnum(row[27]))*1024
            except:
                plano_upload=0
            conexao_tipo = 'ppp'
            ...
            if args.sync_db == True and admmodels.ServicoInternet.objects.filter(login__trim__lower=login).count() == 0:
                ...
                cliente_check = admmodels.Cliente.objects.filter(pessoa__cpfcnpj=cpfcnpj)
                ...
                contrato_check = admmodels.ClienteContrato.objects.filter(cliente__pessoa__cpfcnpj=cpfcnpj)
                # Contrato
	...
	#   python import_csv.py --settings=sgp.netecnologia.settings --nas=1 --portador=1 --vencimentoadd=1 --planoadd=1 --clientes=
