...
if args.clientes:
    ...
    with open(args.clientes, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            idCliente= int(row[0])
            #'%s@import'%
            login= '%s@import'%ustrl(row[24]) + 15000
            if login =='':
            ...
            if plano=='':
                plano ='defautl'
            try:
                plano_valor = int(str(row[21]).replace(',','.'))
            
            except:
                plano_valor=0
            try:
                plano_download=int('200000')
            except:
                plano_download=0
            try:
                plano_upload= int('200000')
            except:
                plano_upload=0
            conexao_tipo = 'ppp'
