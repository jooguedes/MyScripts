...
parser.add_argument('--loginstatus', dest='loginstatus', type=str, help='criar descontos',required=False)
...
if args.planos:
    with open(args.planos, 'rb') as csvfile:
        conteudo = csv.reader(csvfile, delimiter='|', quotechar='"')
        for row in conteudo:
            if admmodels.PlanoInternet.objects.filter(id=row[0]).count() == 0:
                print(row)
                ...
                if 'k' in row[2].lower():
                    ...
                    new_plano_internet.download = row[2] or 0
                ...
                if 'k' in row[3].lower():
                    ...
                    new_plano_internet.upload = row[3] or 0
...
if args.clientes:
    ...
    with open(args.clientes, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            try:
                endereco_inst['numero'] = int(row[13])[:4] # numero_insta
            except:
            ...
            try:
                endereco_cob['numero'] = int(row[13])[:4]  # numero_insta
            except:
            ...
            if ip and '.' not in ip:
                ip = None
            mac_dhcp = row[31]
            planoid= row[32].split(',')
            plano = admmodels.PlanoInternet.objects.filter(id__in=planoid)
            if plano:
                plano = plano[0]
            ...
            if args.sync_db == True and admmodels.ServicoInternet.objects.filter(login=login).count() == 0:
                ...
                if len(cliente_check) == 0:
                    ...
                    try:
                        ...
                        endereco_inst['numero'] = int(0)
                        endereco_cob['numero'] = int(0)
                        new_endereco_cob = admmodels.Endereco(**endereco_cob)
                        new_endereco_inst = admmodels.Endereco(**endereco_inst)
                        new_endereco_cob.save()
                        new_endereco_inst.save()
    ...
    with open(args.titulos, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            if cliente:
                ...
                nosso_numero = titulofunc.TituloFunc().getNossoNumero(portador) + 1 
                if fmodels.Titulo.objects.filter(numero_documento=numero_documento).count() == 0:
                    print row
                    tdata = {} 
                    tdata['cliente'] = cliente
                    tdata['cobranca'] = cobranca
                    tdata['nosso_numero'] = nosso_numero
                    tdata['numero_documento'] = numero_documento
                    tdata['parcela'] = row[6].split('/')[0] # parcela
                    if not row[6]:
                        tdata['parcela'] = 1
                    ...
                    tdata['data_baixa'] = row[10].split(' ')[0]
                    tdata['data_pagamento'] = row[10].split(' ')[0]
                    tdata['data_documento'] = row[11].split(' ')[0]
                    tdata['data_vencimento'] = row[12].split(' ')[0]
                    tdata['data_cancela'] = row[13].split(' ')[0]
                    ...
                    if row[14] == 'C':
                        if not tdata['data_cancela']:
                            ...
                            tdata['status'] = fmodels.MOVIMENTACAO_CANCELADA
                    tdata['djson'] = {}
                    print('idtransacao',row[16],'link', row[17])
                    print tdata
                    if args.sync_db:
