    ...
    for row in conteudo:   
        ...
        if admmodels.ServicoInternet.objects.filter(login=login_secundario).count()==0:
            ...
            try:
                ...
                try:
                    new_endereco= admmodels.Endereco.objects.filter(servicointernet__login=login_primario)[0]
                    new_endereco_cob= copy.copy(new_endereco)
                    new_endereco_intal=copy.copy(new_endereco)
                    new_endereco.save()
                    new_endereco_cob.save()
            ...
            try:
                ...
                print('Erro ao identficar o plano cadastrando novo plano cadastrando plano default')
                planointernet=admmodels.PlanoInternet.objects.filter(plano__id=1)[0]
