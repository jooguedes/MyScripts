...
#python import_receitanet.py --settings=sgp.local.settings --portador=1 --nas=1 --pop=1 --clientes=Conv-clientes-receita.csv --loginsplanos=Conv-mensalidades-receita.csv --sync=1
#python import_receitanet.py --settings=sgp.edsonpaiva.settings --portador=1 --titulos=arquivo.csv 
    ...
    with open(args.clientes, 'rb') as csvfile:
        ...
        for row in conteudo:   
            ...
            if args.sync_db == True and admmodels.ServicoInternet.objects.filter(login__unaccent__trim__lower=login).count() == 0:
                ...
                contrato_check=admmodels.ClienteContrato.objects.filter(id=idcliente)
...
if args.loginsplanos:
    ...
    with open(args.loginsplanos, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            try:
                servico = admmodels.ServicoInternet.objects.filter(login__lower__lower=str(row[0]).lower().strip())
