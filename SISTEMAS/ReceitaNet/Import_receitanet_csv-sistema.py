...
#python import_receitanet.py --settings=sgp.qualitynetba.settings --portador=1 --nas=1 --pop=1 --clientes=Conv-Planilha_Maxs.csv
...
if args.clientes:
    ...
    with open(args.clientes, 'rb') as csvfile:
        ...
        for row in conteudo:   
            ...
            endereco = {}
            endereco['logradouro'] = row[6]
            endereco['pontoreferencia'] = row[7]
            endereco['cidade'] = row[8]
            endereco['uf'] = row[9]
            endereco['bairro'] = row[10]
            endereco['cep'] = row[11]
            endereco['complemento'] = row[12]

            vencimento = row[13]
            telefones = [row[14],row[15],row[16]]
            
            data_nasc=  convertdata(row[18])
            if data_nasc=='':
                data_nasc=None
            data_cadastro = convertdata(row[19])
            if data_cadastro=='':
            ...
            data_ativacao = convertdata(row[21])
            if data_ativacao=='':
                data_ativacao=None
            data_bloqueio = convertdata(row[22])
            if data_bloqueio=='':
                data_bloqueio=None

            data_cancela = convertdata(row[23])
            if data_cancela=='':
                data_cancela=None 
            ...
            plano =row[38]   #'%s - %s' %(row[33],row[37])
            plano_valor = '0.00'
...
if args.loginsplanos:
    ...
    with open(args.loginsplanos, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            try:
                servico = admmodels.ServicoInternet.objects.filter(login__lower=str(row[1]).lower().strip())
...
if args.titulos:
    with open(args.titulos, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            #id_cliente = normalize('NFKD', unicode(row[1])).encode('ASCII','ignore').strip().lower()
            cobranca = fmodels.Cobranca.objects.filter(cliente__clientecontrato__servicointernet__login=row[3])
            if cobranca:
                cliente = cobranca[0].cliente
                ...
                if fmodels.Titulo.objects.filter(portador=portador,
                    ...
                    tdata['valor'] = fnum2(row[7]).replace(',','.')
                    tdata['observacao'] = row[10]
                    tdata['demonstrativo'] = 'Período: %s' %row[5]
                    tdata['valorpago'] = fnum2(row[7]).replace(',','.')
