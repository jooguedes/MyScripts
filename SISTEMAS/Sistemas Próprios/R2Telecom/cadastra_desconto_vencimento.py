...
plano=models.PlanoInternet.objects.all()


for p in plano:
    models.Plano.objects.filter(id=p.plano.id).update(desconto_venc=10)
...
from apps.admcore import models
import csv
with open('/opt/testes_script/Conv-exporta_contratos.csv', 'rb') as csvfile:
