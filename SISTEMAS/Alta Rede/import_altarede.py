...
#python import_altared.py --settings=sgp.i7network.settings --planos= --clientes= --logins= --nas=1 --portador=1 --sync=1
    ...
    with open(args.planos, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            if admmodels.Plano.objects.filter(id=idplano).count() == 0:
                ...
                new_plano_internet.download = row[3]
                try:
                    new_plano_internet.download = int(row[3])
                ...
                try:
                    new_plano_internet.upload = int(row[4])
        ...
        for row in conteudo:
            ...
            for l in logins_dados:
                ...
                try:
                    ...
                    planointernet = admmodels.PlanoInternet.objects.get(id=128)
            ...
            try:
                ...
                if cliente:
                    ...
                    if clientecontrato:
                        ...
                        data_documento = strdate(row[2])
                        nosso_numero = row[1]
                        nosso_numero_f = str(nosso_numero)
                        data_vencimento = strdate(row[2])
                        data_pagamento = strdate(row[3])
                        if not data_pagamento:
                            data_pagamento = None
                        data_baixa = strdate(row[3])
                        valor = row[4]
                        valorpago = row[8]
                        if not valorpago:
                            valorpago = None
