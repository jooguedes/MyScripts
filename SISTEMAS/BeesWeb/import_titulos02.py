...
with open('/tmp/Conv-jedson.csv', 'rb') as csvfile:
    ...
    for row in conteudo:
        ...
        try:
            cliente = admmodels.Cliente.objects.filter(pessoa__cpfcnpj__numfilter=cpfcnpj).order_by('id')
            if not cliente:
            ...
            if cliente:
                ...
                if nosso_numero:
                    ...
                    if ('juno' in titulo.djson.get('link') or 'gerencianet' in titulo.djson.get('link') or 'widepay' in titulo.djson.get('link')) and titulo.portador.gateway_boleto:
