...
# python import_topsap.py --settings=sgp.acmprovedor.settings --portador=6 --nas=1 --pop=1 --arquivo= --planoadd=1 --vencimentoadd=1 --sync=1
...
with open(args.arquivo, 'rb') as csvfile:
    ...
    for row in conteudo:
        ...
        print login,senha,ip,mac,status
        print '####################################################'
        if args.sync_db == True and admmodels.ServicoInternet.objects.filter(login__unaccent__trim__lower=login).count() == 0:
            ...
            try:
                ...
                new_servico.planointernet= admmodels.PlanoInternet.objects.get(plano__id=10)
            new_servico.modoaquisicao = 1 if comodato == True else 0
            new_servico.data_cadastro=data_cadastro
            print(new_servico.login,new_servico.central_password,new_servico.login_password)
            new_servico.save()
