...
'''
python import_controllr.py --settings=sgp.alternativatelecom.settings --nas=controllr-nas.csv
python import_controllr.py --settings=sgp.alternativatelecom.settings --planos=controllr-planos.csv
python import_controllr.py --settings=sgp.alternativatelecom.settings --portadores=controllr-portadores.csv
python import_controllr.py --settings=sgp.alternativatelecom.settings --pop=1 --clientes=controllr-clientes.csv --sync=1
python import_controllr.py --settings=sgp.alternativatelecom.settings --emails=controllr-emails.csv
python import_controllr.py --settings=sgp.alternativatelecom.settings --telefones=controllr-telefones.csv
python import_controllr.py --settings=sgp.alternativatelecom.settings --titulos=controllr-titulos.csv --sync=1
python import_controllr.py --settings=sgp.alternativatelecom.settings --chamados=controllr-chamados.csv
python import_controllr.py --settings=sgp.alternativatelecom.settings --anotacoes=controllr-anotacoes.csv
'''
    ...
    with open(args.portadores, 'rb') as csvfile:
        ...
        for row in conteudo:
            if fmodels.Portador.objects.filter(id=row[0]).count() == 0:
                ...
                new_ponto.id = None
                new_ponto.descricao = row[2][:25]
                new_ponto.portador = fmodels.Portador.objects.filter(descricao=row[2][:25])[0]
                new_ponto.empresa = admmodels.Empresa.objects.all()[0]
                new_ponto.save()
            ...
            if fmodels.GatewayPagamento.objects.filter(portadores__id=row[0]).count() == 0:
                if row[1].strip() in ['gerencianet', 'boleto_facil', 'boletofacil', 'fortunus', 'juno', 'widepay', 'galaxPay']:
                    ...
                    if row[6]:
                        new_gateway_pagamento.usuario = row[5]
                    if row[7]:
                ...
                if len(cliente_check) == 0: 
                    ...
                    try:
                        ...
                        try:
                            ...
                            portador = fmodels.Portador.objects.all()[0]
        ...
        for row in conteudo:
            ...
            if cliente:
                ...
                try:
                    ...
                    contrato = admmodels.ClienteContrato.objects.filter(cliente=cliente)
                if contrato:
                    contrato = contrato[0]
                    ...
                    carne_id = ''
                    carne_link = ''
                    portador = fmodels.Portador.objects.get(id=int(fnum(row[17])))
...
if args.chamados:
    ...
    with open(args.chamados, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            if len(protocolo) > 14:
                protocolo = protocolo[0:8]+protocolo[-4:]
                if amodels.Ocorrencia.objects.filter(numero=protocolo).count() > 0:
                    protocolo = protocolo[0:6]+idOcorrencia
            ...
            if clientecontrato:
                ...
                if amodels.Ocorrencia.objects.filter(id=idOcorrencia).count() == 0:
                    print(row)
                    ocorrencia = {}
                    ocorrencia['id'] = idOcorrencia
...
if args.anotacoes:
    with open(args.anotacoes, 'rb') as csvfile:
        ...
        for row in conteudo:
            ...
            if idContrato != '':
                try:
                    clientecontrato = admmodels.ClienteContrato.objects.filter(id=int(idContrato))[0]
                except Exception as a:
                    print('Erro ao consultar idContrato, erro: %s'%a)
            else:
                try:
                    clientecontrato = admmodels.ClienteContrato.objects.filter(cliente__id=int(idCliente))[0]
                except Exception as a:
                    print('Erro ao consultar idContrato, erro: %s'%a)
