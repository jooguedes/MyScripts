...
parser.add_argument('--arquivo', dest='clientes', type=str, help='Arquivo importacao',required=False)
...
#python import_controlr.py --settings=sgp.plugnetmt.sgp.settings --nas=1 --portador=1 --arquivo='Conv-Autenticação - Lista 2022-10-26-10-43-59-Ativos.csv' --planoadd=1 --vencimentoadd=1 --sync=1
...
# Endereço  Latitude 
# Endereço _ Longitude 
# Contrato 
# Contrato Valor
# Contrato Dia de pagamento 
# Plano

#Cobranças, Financeiro → Cobranças;

#filtrar portadores por forma de pagamento
...
if args.clientes:
    ...
    with open(args.clientes, 'rb') as csvfile:
        ...
        for row in conteudo:
            idcontrato = fnum(row[0].replace(' ', ''))
            if idcontrato != '' and len(idcontrato) < 9:
                ri += 1
                if idcontrato == 0:
                    int(idcontrato+incrementar)

                    
                login= ustrl(row[2])
                if(not login or login==''):
                    login=idcontrato+'semlogin'
                ...
                cep = ustr(row[15]).strip()[0:20]
                uf = ustr(row[14])
                cidade = ustr(row[13]).upper()[0:50]
                ...
                if status == 'Ativo' or 'Bloquueado':
                    status_cc = 1
                    status_s = 1
                    status_c = 1

                    if status == 'Bloquueado':
                        status_cc = 4
                        status_s = 4
                        status_c = 4
                ...
                pop = admmodels.Pop.objects.filter(id=2)[0]
