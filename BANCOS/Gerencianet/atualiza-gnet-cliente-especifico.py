...
idcontrato=912
with open('/tmp/CONTRATO-ID-912.csv', 'rb') as csvfile:
    conteudo = csv.reader(csvfile, delimiter='|', quotechar='"')
    for row in conteudo:
        #print(row)
        cpfcnpj = row[9]
        #print(cpfcnpj)
        clientes = admmodels.Cliente.objects.filter(clientecontrato__id=idcontrato).order_by('id')
        if clientes:
            print(clientes)
            cliente = clientes[0]
            ...
            if contrato:
                ...
                titulo = fmodels.Titulo.objects.filter(portador=1,cliente__clientecontrato__id=idcontrato,
